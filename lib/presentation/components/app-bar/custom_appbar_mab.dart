...
class CustomAppBarWithMAB extends StatefulWidget implements PreferredSizeWidget {
  ...
  const CustomAppBarWithMAB({Key? key, this.isProfileCompleted = false, this.fromAuth = false, this.isTitleCenter = false, this.bgColor = MyColor.primaryColor, this.isShowBackBtn = true, required this.title, this.isShowActionBtn = false, this.actionText = '', this.actionPress1, this.actionPress2, this.isActionIconAlignEnd = false, this.isActionImage = true, this.leadingImage, this.actionImage1, this.actionImage2, this.actionIcon1, this.actionIcon2, this.leadingDrawer = false, this.drawerClick}) : super(key: key);
        ...
        ? AppBar(
            ...
            leading: widget.isShowBackBtn
                ? IconButton(
                    ...
                    icon: widget.leadingImage != null ? SvgPicture.asset(widget.leadingImage!) : Icon(Icons.arrow_back, color: MyColor.getAppBarContentColor(), size: Dimensions.space20))
                : const SizedBox.shrink(),
            backgroundColor: widget.bgColor,
            title: Text(widget.title.tr, style: titleText),
            centerTitle: widget.isTitleCenter,
            actions: [
              ActionButtonIconWidget(pressed: widget.actionPress2!, isImage: widget.isActionImage, icon: widget.actionIcon2 ?? Icons.favorite, imageSrc: widget.actionImage2 ?? "", spacing: Dimensions.space12, size: Dimensions.space22),
              const SizedBox(
                width: 5,
              )
            ],
